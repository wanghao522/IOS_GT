...
@interface PTVBatteryInfo: NSObject
{
...
@interface PTVBatteryHistory : NSObject
{
    CGFloat   _current;
    NSInteger _currentCapacity;
}
...
@interface PTVBattery : NSObject
{
    ...
    PTVBatteryHistory                            *_item; //需要记录的电量信息
    PTVBatteryInfo                               *_info; //查询到到电量相关信息保存
    
}

- (void)handleTick;
@property (nonatomic, assign) NSTimeInterval lastCapacityDate;
@property (nonatomic, retain) PTVBatteryInfo *info;
- (void)resetData;
@end
#endif
