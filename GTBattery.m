...
@implementation PTVBatteryHistory
...
- (id)init
{
    ...
    if (self) {
        ...
        _current = 0;
        _startCapacity = 0;
        _item = [[PTVBatteryHistory alloc] init];
        _info = [[PTVBatteryInfo alloc] init];
...
//        GT_OC_OUT_SET(@"Battery", NO, [NSString stringWithFormat:@"C: %dmAh I: %.1fmA", _startCapacity-[_item currentCapacity], [_item current]]);
       ...
       // GT_OUT_SET("Battery", false, "C: %dmAh I: %.1fmA", _startCapacity-[_item currentCapacity], [_item current]);
        ...
        if (costCapacity != 0) {
            NSTimeInterval interval = 0;//_item.date - _lastCapacityDate;
            if (interval > 0) {
